...
import { getLatestReadings } from "@/lib/utils";
import { fetchReadings, fetchLatestReadings, fetchDevices } from "@/lib/api";
...
export default function Home() {
    ...
    useEffect(() => {
        ...
        const interval = setInterval(async () => {
            try {
                const latestTime =
                    readings.length > 0
                        ? parseUtcDate(readings[readings.length - 1].created_at)
    ...
    const handleRefresh = useCallback(async () => {
        setIsLoading(true);
        try {
            const data = await fetchReadings(15);
            setReadings(data);
            setLastUpdated(new Date());
    ...
    const filteredReadings = useMemo(() => {
        ...
        return readings.filter((r) => r.device_id === Number(selectedDeviceId));
    ...
    const kpiData = useMemo(() => {
        ...
        const avgDba =
            allLatest.reduce((sum, r) => sum + r.avg_dba, 0) / allLatest.length;
        ...
        const recentReadings = filteredReadings.filter(
            (r) => parseUtcDate(r.created_at) > fiveMinAgo
        );
        const peakCount = recentReadings.reduce((sum, r) => {
            ...
            <div className="container mx-auto p-6 space-y-6">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                    ...
                    <KPICard
                        ...
                        variant={
                            kpiData.peakCount > 50
                                ? "high"
                                : kpiData.peakCount > 20
                                    ? "elevated"
                                    : "default"
                        }
                        loading={isLoading}
